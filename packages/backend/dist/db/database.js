...
const sqlSchema_1 = require("./sqlSchema");
const DATA_DIR = path_1.default.resolve(__dirname, '../../data');
const SQLITE_FILE = path_1.default.join(DATA_DIR, 'caretrace.sqlite');
const LEDGER_FILE = path_1.default.join(DATA_DIR, 'caretrace.ledger.json');
class Database {
    isPostgres;
    pgKnex;
    sqlite;
    ...
    // -------------------------------------------------------------
    initSqlite() {
        this.sqlite = new better_sqlite3_1.default(SQLITE_FILE);
    ...
    async syncFromPostgres() {
        ...
        try {
            ...
            this.pgCache.users = usersRows.map(sqlSchema_1.rowToUser);
            const instRows = await this.pgKnex('institutions').select('*');
            this.pgCache.institutions = instRows.map(sqlSchema_1.rowToInstitution);
            const reqRows = await this.pgKnex('requirements').select('*');
            this.pgCache.requirements = reqRows.map(sqlSchema_1.rowToRequirement);
            const donRows = await this.pgKnex('donations').select('*');
            this.pgCache.donations = donRows.map(sqlSchema_1.rowToDonation);
            const annRows = await this.pgKnex('announcements').select('*').orderBy('created_at', 'desc');
            this.pgCache.announcements = annRows.map(sqlSchema_1.rowToAnnouncement);
            const riskRows = await this.pgKnex('risk_audit_logs').select('*').orderBy('triggered_at', 'desc');
            this.pgCache.riskAuditLogs = riskRows.map(sqlSchema_1.rowToRiskFlag);
            const telRows = await this.pgKnex('transit_telemetry').select('*');
            this.pgCache.transitTelemetry = {};
            telRows.forEach((r) => {
                const tel = (0, sqlSchema_1.rowToTelemetry)(r);
    ...
    loadLedger() {
        try {
            if (fs_1.default.existsSync(LEDGER_FILE)) {
                ...
                const legacyFile = path_1.default.join(DATA_DIR, 'caretrace.db.json');
    ...
    saveLedger() {
        try {
            this.ensureDataDir();
            fs_1.default.writeFileSync(LEDGER_FILE, JSON.stringify(this.ledgerBlocks, null, 2), 'utf-8');
        }
        catch (e) {
    ...
    getUsers() {
        ...
        return rows.map(sqlSchema_1.rowToUser);
    }
    getUserById(id) {
        if (this.isPostgres) {
    ...
    getInstitutions() {
        ...
        return rows.map(sqlSchema_1.rowToInstitution);
    }
    ...
    getRequirements() {
        ...
        return rows.map(sqlSchema_1.rowToRequirement);
    }
    ...
    getDonations() {
        ...
        return rows.map(sqlSchema_1.rowToDonation);
    }
    getDonationById(id) {
        if (this.isPostgres) {
        ...
        return row ? (0, sqlSchema_1.rowToDonation)(row) : undefined;
    }
    ...
    getAnnouncements() {
        ...
        return rows.map(sqlSchema_1.rowToAnnouncement);
    }
