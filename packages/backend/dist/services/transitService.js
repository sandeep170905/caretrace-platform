...
class TransitService {
     ...
     */
    static generateWaypoints(start, end, count = 10) {
        const waypoints = [];
     ...
     */
    static calculateDistanceKm(c1, c2) {
     ...
     */
    static getOrInitTelemetry(donationId) {
        const existing = database_1.db.getTransitTelemetry(donationId);
        if (existing)
            return existing;
        ...
        const totalDistance = this.calculateDistanceKm(donation.pickupCoordinates, donation.destinationCoordinates);
        ...
        database_1.db.upsertTransitTelemetry(initialTelemetry);
        return initialTelemetry;
    }
    /**
     * Advances the simulation progress by an increment
     */
    static advanceSimulation(donationId, incrementPercent = 20) {
        const donation = database_1.db.getDonationById(donationId);
        if (!donation)
            throw new Error(`Donation ${donationId} not found`);
        ...
        const waypoints = this.generateWaypoints(donation.pickupCoordinates, donation.destinationCoordinates, 20);
        const targetIdx = Math.min(waypoints.length - 1, Math.round((newProgress / 100) * (waypoints.length - 1)));
        ...
        database_1.db.upsertTransitTelemetry(updated);
