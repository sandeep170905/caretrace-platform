...
async function runStageFAudit() {
    ...
    const newDonor = {
        ...
        passwordHash: authService_1.AuthService.hashPassword('Pass@1234'),
        createdAt: now
    };
    database_1.db.upsertUser(newDonor);
    const registeredDonor = database_1.db.getUserById(newDonor.id);
    ...
    const directorUser = {
        ...
        passwordHash: authService_1.AuthService.hashPassword('DavidPass2026'),
        createdAt: now
    };
    database_1.db.upsertUser(directorUser);
    // Verify institution is in pending list
    const pendingInstitutions = database_1.db.getInstitutions().filter(i => !i.verified);
    ...
    database_1.db.upsertInstitution(newInstitution);
    const approvedInst = database_1.db.getInstitutionById(instId);
    ...
    const scoringResult = fraudScoringService_1.FraudScoringService.evaluateAndLogRequirement(newReqData, instId);
    ...
    database_1.db.upsertRequirement(createdReq);
    const retrievedReq = database_1.db.getRequirementById(reqId);
    ...
    const qrCodePayload = qrService_1.QRService.createPayloadString(donationId, newDonor.id, instId);
    ...
    const genesisBlock = ledgerService_1.LedgerService.recordCheckpoint(donationId, 'DONATION_MATCHED', { id: newDonor.id, role: 'DONOR', name: newDonor.name }, `${newDonor.name} pledged ${pledgeQuantity} ${createdReq.unit} to ${newInstitution.name}.`, { items: newDonation.items, donorId: newDonor.id });
    ...
    const agent = database_1.db.getUserById('user-agent-sakthivel');
    ...
    database_1.db.upsertDonation(newDonation);
    const pickupBlock = ledgerService_1.LedgerService.recordCheckpoint(donationId, 'PICKUP_VERIFIED', { id: agent.id, role: 'PICKUP_AGENT', name: agent.name }, `Pickup authenticated by courier ${agent.name} at Adyar hub. Sealed for transit.`, { agent: agent.name, pickupHub: newDonation.pickupAddress });
    const inTransitBlock = ledgerService_1.LedgerService.recordCheckpoint(donationId, 'IN_TRANSIT_CHECKPOINT', { id: 'system-iot', role: 'SYSTEM', name: 'CareTrace Route Telemetry Engine' }, `Consignment in corridor: Grand Southern Trunk Road passing Guindy. Speed: 42 km/h.`, { speedKmh: 42, progress: '60%' });
    ...
    database_1.db.upsertDonation(newDonation);
    const deliveryBlock = ledgerService_1.LedgerService.recordCheckpoint(donationId, 'DELIVERY_CONFIRMED', { id: directorUser.id, role: 'INSTITUTION', name: directorUser.name }, `Consignment delivered and inspected by ${directorUser.name} (${newInstitution.name}). Attached tamper-evident proof.`, { signature: newDonation.recipientSignature, notes: newDonation.confirmationNotes });
    const certificate = {
        donationId,
        donorName: newDonor.name,
        institutionName: newInstitution.name,
    ...
    const ledgerVerification = ledgerService_1.LedgerService.verifyChain();
