...
async function runStageABVerification() {
    ...
    const retrievedUser = database_1.db.getUserById(newUser.id);
    ...
    const newDonorDonations = database_1.db.getDonationsByDonor(newUser.id);
    const ajithDonations = database_1.db.getDonationsByDonor('user-donor-ajith');
    ...
    const institution = database_1.db.getInstitutionById(targetReq.institutionId);
    const donationId = `CT-2026-${Math.floor(1000 + Math.random() * 9000)}`;
    const qrCodePayload = qrService_1.QRService.createPayloadString(donationId, newUser.id, institution.id);
    ...
    ledgerService_1.LedgerService.recordCheckpoint(donationId, 'DONATION_MATCHED', { id: newUser.id, role: 'DONOR', name: newUser.name }, `Donation initiated by ${newUser.name} and matched to requirement: "${targetReq.title}"`, { items: newDonation.items, institution: institution.name });
    ...
    const updatedNewDonorDonations = database_1.db.getDonationsByDonor(newUser.id);
    const updatedAjithDonations = database_1.db.getDonationsByDonor('user-donor-ajith');
    ...
    const passwordValid = authService_1.AuthService.verifyPassword(testPassword, userAfterRestart.passwordHash);
