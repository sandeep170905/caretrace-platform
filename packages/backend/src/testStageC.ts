...
import { numberToIndianWords, TaxExemptionReceipt, Donation } from '@caretrace/shared';
...
async function runStageCVerification() {
  ...
  const donor = db.getUsers().find(u => u.role === 'DONOR')!;
  const requirement = db.getRequirements()[0];
  ...
  const verify = LedgerService.verifyChain();
