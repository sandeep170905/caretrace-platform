...
import { User, Institution, Requirement, Donation, Announcement } from '@caretrace/shared';
...
async function runFullPersistenceVerification() {
  ...
  await db.upsertInstitution(testInstitution);
  ...
  await db.upsertRequirement(testReq);
  ...
  await db.upsertDonation(testDonation);
  LedgerService.recordCheckpoint(
    testDonationId,
    'DONATION_MATCHED',
    { id: donorUser.id, role: 'DONOR', name: donorUser.name },
    `Priya Ramanathan pledged 10 digital learning tablets to ${testInstitution.name}.`,
    { itemsCount: 10, totalValueInr: 90000 }
  );
  ...
  await db.upsertAnnouncement(testAnnouncement);
  ...
  if (engine === 'sqlite') {
    ...
    const sqlitePath = path.join(__dirname, '../data/caretrace.sqlite');
  ...
  const authSuccess = AuthService.verifyPassword(testPassword, retrievedDonor?.passwordHash!);
